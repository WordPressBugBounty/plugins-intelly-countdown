 ...
 * @since   3.7.0
 */
final class ICP_Session extends ICP_Recursive_ArrayAccess implements Iterator, Countable {
	/**
	...
	protected function __construct() {
		if ( isset( $_COOKIE[ WP_SESSION_COOKIE ] ) ) {
			$cookie        = stripslashes( sanitize_text_field( wp_unslash( $_COOKIE[ WP_SESSION_COOKIE ] ) ) );
			...
			if ( time() > $this->exp_variant ) {
				$this->set_expiration();
				$this->session_id = sanitize_text_field( $this->session_id );
				$this->expires = sanitize_option('date_format', $this->expires );
